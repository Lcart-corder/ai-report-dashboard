...
import { projectDetail as pd } from "@/lib/mock";
import {
  Card,
  CardHeader,
  ProgressBar,
...
export default function ProjectDetailPage() {
  ...
  if (!project) {
    return (
      <div className="mx-auto max-w-2xl p-10 text-center">
        ...
        <p className="mt-1 text-sm text-slate-400">削除されたか、URLが正しくない可能性があります。</p>
        <Link href="/projects" className="mt-4 inline-block text-sm font-medium text-blue-600">
          ← プロジェクト一覧へ戻る
        </Link>
      </div>
    );
  ...
  const dueDate = project.due ? new Date(project.due.replace(/\//g, "-")) : null;
  const daysLeft = dueDate ? Math.round((dueDate.getTime() - APP_TODAY.getTime()) / 86400000) : null;
  ...
  return (
    <div className="mx-auto max-w-[1500px] p-4 md:p-6">
        ...
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div>
            <div className="flex items-center gap-2">
              ...
              <span className="text-xs text-slate-400">{project.bukai}</span>
            </div>
            <h1 className="mt-2 text-3xl font-bold text-slate-800">{project.name}</h1>
          </div>
          <div className="flex gap-2">
            ...
            >
              <IconTrash width={16} height={16} /> 削除
            </button>
          </div>
        </div>
        ...
        <div className="mt-5 grid grid-cols-1 gap-6 md:grid-cols-3">
          <div>
            ...
            <div className="mt-1 flex items-center gap-3">
              <span className="text-3xl font-bold text-slate-800">{project.progress}%</span>
              <ProgressBar value={project.progress} className="w-full max-w-[220px]" />
            </div>
          </div>
          <div>
            <p className="text-sm text-slate-400">責任者</p>
            <div className="mt-1 flex items-center gap-2">
              <Avatar name={project.owner || "?"} size={34} />
              <div>
                <p className="text-sm font-semibold text-slate-800">{project.owner || "未設定"}</p>
                <p className="text-xs text-slate-400">{pd.ownerTitle}</p>
              </div>
            </div>
          </div>
          <div>
            ...
            <div className="mt-1 flex items-center gap-2">
              ...
              <div>
                <p className="text-sm font-semibold text-slate-800">{project.due || "未設定"} {project.due && "まで"}</p>
                {daysLeft !== null && (
                  <p className={`text-xs font-medium ${daysLeft < 0 ? "text-red-500" : "text-slate-500"}`}>
                    {daysLeft < 0 ? `${-daysLeft}日 超過` : `残り ${daysLeft}日`}
                  </p>
                )}
              </div>
            </div>
        ...
        <div className="space-y-4 lg:col-span-2">
          <Card className="px-5 py-2">
            <div className="flex items-center justify-between pt-2">
              <span className="text-xs text-slate-400">以下はサンプル項目です（試作版）</span>
            </div>
            <Row label="概要">{pd.summary}</Row>
            <Row label="目的">{pd.purpose}</Row>
            <Row label="対象者">{pd.target}</Row>
            <Row label="予算">
        ...
        <div className="space-y-4">
          <Card className="p-5">
            ...
            <div className="mt-3 flex items-center gap-2">
              <span className="font-semibold text-slate-800">{pd.latestMeeting.title}</span>
              <StatusBadge status={pd.latestMeeting.tag} />
            </div>
            <p className="mt-1 text-xs text-slate-400">{pd.latestMeeting.date}</p>
            <dl className="mt-3 space-y-2 text-sm">
              <div className="flex gap-2">
                <dt className="shrink-0 text-slate-400">出席者</dt>
                <dd className="text-slate-600">{pd.latestMeeting.attendees}</dd>
              </div>
              <div className="flex gap-2">
                <dt className="shrink-0 text-slate-400">議題</dt>
                <dd className="text-slate-600">{pd.latestMeeting.topics}</dd>
              </div>
              <div className="flex gap-2">
                <dt className="shrink-0 text-slate-400">次回予定</dt>
                <dd className="text-slate-600">{pd.latestMeeting.next}</dd>
              </div>
            </dl>
          </Card>
          ...
          <Card className="border-blue-200 bg-blue-50/50 p-5">
            ...
            <p className="mt-3 text-sm leading-relaxed text-slate-600">{pd.aiComment}</p>
      ...
      <ProjectFormModal open={editing} initial={editing ? project : null} onClose={() => setEditing(false)} />
    </div>
  );
}
