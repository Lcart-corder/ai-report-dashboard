...
import { IconPlus, IconDownload, IconDots, IconSettings } from "@/components/icons";
...
export default function ProjectsPage() {
  ...
  const bukaiList = useMemo(
    () => ["すべての部会", ...Array.from(new Set(projects.map((p) => p.bukai)))],
    []
  );
        ...
        <div className="grid grid-cols-1 gap-4 md:grid-cols-4">
          ...
          <div>
            ...
            <Select value={status} onChange={(e) => setStatus(e.target.value)}>
              {["すべてのステータス", "進行中", "計画中", "完了", "未着手"].map((s) => (
          ...
          <div>
            ...
            <Select defaultValue="すべての担当者">
              ...
              {Array.from(new Set(projects.map((p) => p.owner))).map((o) => (
      ...
      <Card className="overflow-hidden">
        ...
        <div className="flex items-center justify-between px-5 py-3 text-sm text-slate-500">
          <div className="flex items-center gap-1">
            ...
            <button className="rounded-lg px-3 py-1 hover:bg-slate-100">2</button>
            <button className="rounded-lg px-2 py-1 hover:bg-slate-100">»</button>
          </div>
          <span>
