...
// 10. Integrations
export interface IntegrationShopify {
  id: ID;
  tenant_id: ID;
  shop_url: string; // e.g., "my-shop.myshopify.com"
  access_token: string;
...
export interface IntegrationLineOfficial {
  id: ID;
  tenant_id: ID;
  channel_id: string;
  ...
  status: 'active' | 'inactive' | 'error';
  bot_info_json?: JSONValue; // Cached bot profile
  created_at: DateTime;
  updated_at: DateTime;
}

export interface IntegrationLineAds {
  id: ID;
  tenant_id: ID;
  line_tag_id: string;
  enable_conversion_tracking: boolean;
  enable_audience_sync: boolean;
