 ...
 */
function setupSheet(ss, sheetName, headers, dataRows) {
  var sheet = ss.getSheetByName(sheetName);

  if (sheet) {
    // 既存シートがある場合、ヘッダーが空なら追加
...
/**
 * テスト用: APIの動作確認
 * ブラウザで {DEPLOY_URL}?action=getStopCodes にアクセスして確認できる
 */
function testApi() {
  var result = getStopCodes({});
  Logger.log(JSON.stringify(result, null, 2));

  var user = getCurrentUser({ email: 'admin@lcart-official.co.jp' });
